...
test("Discipline Apex reaches both consumers with separate native damage packets",async()=>{
  ...
  for(const [discipline,type] of [["cryokinesis","cold"],["pyrokinesis","fire"],["psychokinesis","force"],["electrokinesis","lightning"]] as const){
    assert.deepEqual(harness.core.psionic_apex[`${discipline}_manifested_strike_hit`],{discipline_id:discipline,uses_per_attack_action:1,reset:"start_of_each_attack_action",declaration:"before_attack_roll",consumption:"on_declaration",damage_type:type,damage:{kind:"dice",count:3,sides:8},critical_dice_multiplier:1,psi_cost:0,blood_tax:0});
  }
  ...
  for(const phrase of ["Discipline Maturation", "before making a Manifested Strike attack roll", "The use is spent when you declare it, even if the attack misses", "If that declared attack hits a creature", "3d8 damage to that creature", "cold for Cryokinesis, fire for Pyrokinesis, force for Psychokinesis, and lightning for Electrokinesis", "This packet has no Overload tier", "even when the strike delivers a Tier-2 rider or uses the Holdout Option"]){assert.ok(prose.includes(phrase),phrase);}
});
...
test("Apex cannot drift to per-strike, resource-paid, critical-doubled, or incorrect damage types",async()=>{
  ...
  for(const [key,value] of [["declaration","after_attack_roll"],["consumption","on_hit"],["reset","start_of_turn"],["uses_per_attack_action",2],["critical_dice_multiplier",2],["damage_type","acid"],["psi_cost",1],["blood_tax",1]] as const){
