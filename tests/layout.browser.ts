...
test("Subclass Feature Reference keeps both tables readable across screen and print layouts",async()=>{
  ...
  for(const engine of desktopBrowsers){
    ...
    try{
      ...
      assert.equal(filtered.totalRows,33);assert.equal(filtered.visibleRows,0);assert.equal(filtered.headerDisplay,"table-header-group");assert.equal(filtered.count,"Showing 0 of 33 features.");assert.equal(filtered.noMatch,false);assert.equal(filtered.scrollable,true);assert.equal(filtered.noForcedAriaHiding,true);
      ...
      assert.equal(printLayout.rowDisplays.length,33);assert.ok(printLayout.rowDisplays.every(display=>display==="table-row"),engine.name+" print restores all rows");assert.equal(printLayout.controlsDisplay,"none",engine.name+" print hides table filters");
...
test("concentration metadata ribbon wraps without overflow on desktop and mobile",async()=>{
  const result=await executeBuild("prototype");const url=`${pathToFileURL(result.htmlPath).href}#calculator&card=advanced_barrier&level=20&modifier=5&group=advanced_training`;
  const browser=await chromium.launch(browserLaunchOptions);
  try{
    for(const viewport of [{width:1366,height:768},{width:390,height:844}]){
  ...
  for(const engine of desktopBrowsers){
    ...
    try{
      for(const viewport of [{name:"desktop",width:1280,height:1000,columns:3},{name:"mobile",width:390,height:844,columns:1}] as const){
        ...
        await page.locator('.calculator__card[data-card-id="flare"]').click();await page.waitForFunction(()=>document.querySelector("#calculator-feature-results > h3")?.textContent==="Flare");const flareLayout=await page.evaluate(()=>{const root=document.querySelector<HTMLElement>("#calculator-root")!,result=document.querySelector<HTMLElement>("#calculator-feature-results")!,rootRect=root.getBoundingClientRect(),core=result.querySelector<HTMLElement>(".calculator__core")!,saveGroups=result.querySelector<HTMLElement>(".calculator__save-groups")!,saveGroup=result.querySelector<HTMLElement>(".calculator__save-group")!,blocks=[result,core,saveGroups,saveGroup];return{coreHeading:core.querySelector(":scope > h4")?.textContent,saveText:saveGroup.textContent,contained:blocks.every(element=>{const rect=element.getBoundingClientRect();return rect.left>=rootRect.left-1&&rect.right<=rootRect.right+1&&element.scrollWidth<=element.clientWidth+1;}),overflow:document.documentElement.scrollWidth-document.documentElement.clientWidth};});assert.equal(flareLayout.coreHeading,"Manifested Strike",context+" rider core heading");assert.match(flareLayout.saveText??"",/Dexterity save · Tiers 0–2/u,context+" grouped save scope");assert.equal(flareLayout.contained,true,context+" core/save containment");assert.equal(flareLayout.overflow,0,context+" core/save overflow");
...
test("Start Here is a contained single-column experience across engines, breakpoints, input, forced colors, reduced motion, and print",async()=>{
  ...
  for(const engine of desktopBrowsers){
    ...
    try{
      ...
      await page.keyboard.press("Tab");assert.equal(await page.evaluate(()=>document.activeElement?.getAttribute("data-onboarding-link-id")),"primary_build",engine.name+" primary Start Here path follows view navigation");
      ...
      await page.focus('[data-onboarding-link-id="term_blood_tax"]');assert.equal(await page.evaluate(()=>document.activeElement?.getAttribute("data-onboarding-link-id")),"term_blood_tax",engine.name+" Blood Tax glossary keyboard target");await page.keyboard.press("Enter");await page.waitForFunction(()=>document.querySelector("#calculator-feature-results > h3")?.textContent==="Blood Tax");
      assert.match(new URL(page.url()).hash,/^#calculator&card=blood_tax&level=20&modifier=5&group=common_features$/u,engine.name+" Blood Tax glossary keyboard route");assert.equal(await page.locator('.calculator__card[aria-pressed="true"]').getAttribute("data-card-id"),"blood_tax",engine.name+" Blood Tax glossary keyboard selection");assert.equal(await page.locator("#calculator-feature-results .calculator__tier").count(),3,engine.name+" Blood Tax glossary keyboard detail");
      await page.focus("#view-start-here");await page.keyboard.press("Enter");
      const touchPage=await browser.newPage({viewport:{width:412,height:915},hasTouch:true});try{await touchPage.goto(homeUrl);await touchPage.locator('[data-onboarding-link-id="term_blood_tax"]').tap();await touchPage.waitForFunction(()=>document.querySelector("#calculator-feature-results > h3")?.textContent==="Blood Tax");assert.match(new URL(touchPage.url()).hash,/^#calculator&card=blood_tax&level=20&modifier=5&group=common_features$/u,engine.name+" Blood Tax glossary touch route");assert.equal(await touchPage.locator('.calculator__card[aria-pressed="true"]').getAttribute("data-card-id"),"blood_tax",engine.name+" Blood Tax glossary touch selection");}finally{await touchPage.close();}
