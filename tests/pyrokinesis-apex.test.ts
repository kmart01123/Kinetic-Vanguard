...
test("Pyrokinesis Apex reaches both consumers with its separate fire packet",async()=>{
  ...
  assert.deepEqual(harness.core.psionic_apex.pyrokinesis_manifested_strike_hit,{discipline_id:"pyrokinesis",uses_per_attack_action:1,reset:"start_of_each_attack_action",damage_type:"fire",damage:{kind:"dice",count:3,sides:8},critical_dice_multiplier:1,psi_cost:0,blood_tax:0});
  ...
  for(const phrase of ["Pyrokinesis Maturation", "3d8 fire damage to that creature", "This packet has no Overload tier", "even when the strike delivers a Tier-2 rider or uses the Holdout Option"]){assert.ok(prose.includes(phrase),phrase);}
});
