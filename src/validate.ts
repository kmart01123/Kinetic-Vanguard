...
export function validateSemantics(authority:Authority):Diagnostic[]{
  ...
  for(const [entityIndex,entity] of authority.entities.entries()){
    ...
    for(const [surfaceIndex,surface] of (entity.mechanics?.surfaces??[]).entries()){
      ...
      if(surface.damage_options){
        ...
        for(const [optionIndex,option] of surface.damage_options.entries()){
          const path=`${surfacePath}/damage_options/${optionIndex}`;
          if(option.minimum_level<Number(entity.level)||option.minimum_level>20)diagnostics.push({severity:"error",code:"mechanics.damage_option_level",message:`${entity.id} damage option must unlock between the feature level and Fighter 20`,path});
          if(option.targeting.kind!=="struck_target"||option.targeting.topology!=="single")diagnostics.push({severity:"error",code:"mechanics.damage_option_targeting",message:`${entity.id} damage options must forgo all additional targets`,path});
        }
      }
  ...
  if(authority.entities.length!==43)diagnostics.push({severity:"error",code:"onboarding.entity_boundary",message:`Onboarding must remain outside the 43-entity publication boundary; found ${authority.entities.length} entities`,path:"/entities"});
