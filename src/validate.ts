...
export function validateSemantics(authority:Authority):Diagnostic[]{
  ...
  const expectedApex={minimum_level:18,psychokinesis_manifested_strike_hit:{discipline_id:"psychokinesis",uses_per_attack_action:1,reset:"start_of_each_attack_action",damage_type:"force",damage:{kind:"dice",count:3,sides:8},critical_dice_multiplier:1,psi_cost:0,blood_tax:0},pyrokinesis_manifested_strike_hit:{discipline_id:"pyrokinesis",uses_per_attack_action:1,reset:"start_of_each_attack_action",damage_type:"fire",damage:{kind:"dice",count:3,sides:8},critical_dice_multiplier:1,psi_cost:0,blood_tax:0}};
  if(JSON.stringify(harness.psionic_apex)!==JSON.stringify(expectedApex))diagnostics.push({severity:"error",code:"harness.psionic_apex",message:"Psionic Apex must retain the once-per-Attack-action Psychokinesis and Pyrokinesis damage packets",path:"/calculator/harness_mechanics/psionic_apex"});
