...
The CLI envelope is checked before selecting a review payload. Explicit error flags, structured-output errors, non-success result subtypes, and any non-null reported stop reason other than `end_turn` reject the output even if it contains a valid-looking PASS. Unknown stop reasons are rejected without echoing arbitrary provider text. Envelopes with absent/null stop metadata and legacy flat contracts remain supported and still undergo contract and finality checks; missing metadata does not prove that inspection occurred. A completed structured result takes precedence over accumulated progress text. See the [upstream Grok headless protocol](https://github.com/xai-org/grok-build/blob/72a61251fcffb464bcc687aeb5a998e5a98ec0c9/crates/codegen/xai-grok-pager/docs/user-guide/14-headless-mode.md#json) for the envelope fields.
...
The tool-name correction is based on Grok's documented native IDs and its [session tool filter](https://github.com/xai-org/grok-build/blob/72a61251fcffb464bcc687aeb5a998e5a98ec0c9/crates/codegen/xai-grok-agent/src/config.rs#L1333), which uses exact full or short IDs. Earlier runtime probe evidence above applies to the versions named there. The correction and CLI completion checks have offline regression coverage; no new live model review was run for these changes.
