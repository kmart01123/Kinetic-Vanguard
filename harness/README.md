...
The surgical damage integration keeps Absolute Zero's six dice and fixed bonus in one exact distribution before save halving and defense rounding. Telekinetic Shove resolves its canonical fixed-damage threshold at the current Fighter level. Branching Bolt's Fighter-18 Focused Bolt is a separate pre-roll choice under the same feature and tier, with the same Psi and Blood Tax costs and exactly one target. It competes with ordinary branching at every cluster size; unused branches never multiply its two-die packet. The native harness projection is 1.9.0 (authority schema 2.13.0, rules 15.0.0). See [integration evidence](../docs/surgical-damage-integration.md).
